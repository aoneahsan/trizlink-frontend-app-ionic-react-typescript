...
const ZaionsAPIKeysTable: React.FC = () => {
  ...
  const showActionsPopover = (
    ...
    if (actionsPopoverRef.current !== null) {
      actionsPopoverRef.current.event = _event;
    }
  };

  const editApiKeyDetails = async (): Promise<void> => {
    try {
      if (
        compState?.selectedApiKey !== null &&
        compState?.selectedApiKey?.trim()?.length > 0 &&
        apiKeyData?.length > 0
      ) {
  ...
  const deletePixelAccount = async (): Promise<void> => {
    try {
      if (
        compState?.selectedApiKey !== null &&
        compState?.selectedApiKey?.trim()?.length > 0 &&
  ...
  const removePixelAccount = async (): Promise<void> => {
