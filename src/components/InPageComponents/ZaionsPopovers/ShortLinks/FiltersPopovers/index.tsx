    ...
    try {
      setShortLinksFilterOptions(oldValues => ({
        ...
        timeFilter: {
          ...
          startedAt: _startedAt ?? oldValues?.timeFilter?.startedAt,
          endAt: _endAt ?? oldValues?.timeFilter?.startedAt
        }
      }));
    } catch (error) {
  ...
  return (
    <ZRScrollbars className='w-[300px] h-[300px]'>
      <Formik
        ...
        onSubmit={values => {
          try {
            if (values?._filteredTags !== undefined) {
              const _tags: string[] = [];
...
export const ShortLinksDomainsFiltersPopover: React.FC = () => {
  ...
  const generateInitialValueOfDomainsFormik = (
    ...
    _filteredDomains?: Record<string, boolean>;
    _allDomains?: boolean;
  } => {
    try {
      const _filteredDomains: Record<string, boolean> = {};
  ...
  return (
    <ZRScrollbars className='w-[300px] h-[300px]'>
      <Formik
        ...
        onSubmit={values => {
          try {
            if (values?._filteredDomains !== undefined) {
          ...
          <>
            ...
            <ZIonItem className='ion-no-padding'>
              ...
              <ZRCheckbox
                ...
                handleChange={checked => {
                  void setFieldValue('_allDomains', checked, false);
                ...
                return (
                  <ZIonItem key={i}>
                    ...
                    <ZIonCheckbox
                      slot='end'
                      checked={
                        values?._filteredDomains !== undefined &&
                      ...
                      onIonChange={({ target }) => {
                        if (
                          !target.checked &&
                          values?._allDomains !== undefined
                        ) {
                          void setFieldValue('_allDomains', false, false);
                        }
                        void setFieldValue(
                          `_filteredDomains.${domain}`,
                          target.checked,
