    ...
    <>
      <Dropdown>
        <Dropdown.Toggle
          ...
          className={`${classes.zaions__dropdown_toggle} text-primary d-flex ion-align-items-center`}
        >
          {newShortLinkTypeOptionData && (
            <>
              ...
              <ZIonText
                ...
                color={'dark'}
              >
                <h6 className='ion-no-margin fw-bold d-inline'>
        ...
        <Dropdown.Menu className={`ms-auto ${classes.zaions_shorturl_options}`}>
          <ZIonGrid>
            <ZIonRow className='zaions__w100'>
              {ShortUrlLinkOptionData.map((el) => {
                return (
                  <ZIonCol size='5' key={el.id}>
                    <ZIonText
                      className={`d-flex gap-3 ion-align-items-center ion-margin-bottom ${classes.zaions_cursor__pointer}`}
                      onClick={() => {
